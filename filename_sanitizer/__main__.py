...
def sanitize_filename(filename, separator, table=None):
...
def rename_files_recursive(
    ...
    for root, dirs, files in os.walk(dirname, topdown=False):
        for name in files:
            ...
            new_name = sanitize_filename(name, separator, table)
            new_name = os.path.join(
                root,
                sanitize_filename(name, separator, table)
            )
...
def main():
    ...
    if not args.file:
        print(sanitize_filename(args.filename, args.separator, table))
        return 0
    ...
    if args.recursive:
        ...
        rename_file(
            args.filename,
            sanitize_filename(old_name, args.separator, args.table),
            args.interactively,
            args.verbose
        )
        return 0
